...
*   **PERMISSION DENIED / UID MATCH:** Logstash will crash-loop if the host user's UID doesn't match the internal container expectations. 
    ...
    *   *Location:* **192.168.1.21** uses UID **1003**. You must manually update `docker-compose-backend.yml` to include `user: "1003:1003"` if redeploying there.
...
- **Backends (192.168.1.21, 192.168.1.90)**:
...
1.  **Configure Environment**:
    `./deploy.sh --generate`
2.  **Deploy Frontend (192.168.1.10)**:
    `./deploy.sh --frontend`
...
Adding new switches (e.g., a new Juniper at 10.0.0.96) is now automatic:
...
- **Fix:** Re-apply the frontend template and manually rollover the index:
  `curl -k -u elastic:telehouse -X POST "https://192.168.1.10:9200/logstash-flow-write/_rollover"`
